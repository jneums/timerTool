...
module {
  ...
  public type ReconstitutionTrace = MigrationTypes.Current.ReconstitutionTrace;
  public type TimerDiagnostics = MigrationTypes.Current.TimerDiagnostics;
  public type ActionFilter = MigrationTypes.Current.ActionFilter;
  public type CancellationResult = MigrationTypes.Current.CancellationResult;
  ...
  public type ClassPlus = ClassPlusLib.ClassPlus<TimerTool, State, InitArgs, Environment>;
  ...
  public func Init<system>(
    ...
    ClassPlusLib.ClassPlus<system, TimerTool, State, InitArgList, Environment>({
      config with constructor = TimerTool
    }).get;
  };
  ...
  public class TimerTool(stored : ?State, caller : Principal, canister : Principal, args : ?InitArgList, _environment : ?Environment, storageChanged : (State) -> ()) {
    ...
    private func shareCycles2<system>() : async () {
      ...
      if (state.nextActionId > 0) {
        state.lastActionIdReported := ?(state.nextActionId - 1);
      } else {
    ...
    private func executeActions<system>() : async () {
      ...
      var actionsToExecute = BTree.scanLimit<ActionId, Action>(
        state.timeTree,
        ActionIdCompare,
        minAction.0,
        {
      ...
      label proc for (thisAction in actionsToExecute.results.vals()) {
        ...
        let executionHandler = switch (Map.get(executionListeners, Map.thash, thisAction.1.actionType)) {
          case (?val) val;
          case (null) {
            switch (Map.get(executionListeners, Map.thash, "")) {
    ...
    // Utility Functions for Tracing Reconstitution

    /// Record a reconstitution trace event for debugging and monitoring
    public func addReconstitutionTrace(fromVersion : Text, toVersion : Text, actionsRestored : Nat, errors : [Text]) : () {
      let now = get_time();
      let trace : ReconstitutionTrace = {
        timestamp = now;
        migratedFrom = fromVersion;
        migratedTo = toVersion;
        actionsRestored = actionsRestored;
        timersRestored = BTree.size(state.timeTree);
        validationPassed = errors.size() == 0;
        errors = errors;
      };
      state.reconstitutionTraces := Array.append(state.reconstitutionTraces, [trace]);
    };

    /// Get all reconstitution traces for debugging
    public func getReconstitutionTraces() : [ReconstitutionTrace] {
      state.reconstitutionTraces;
    };

    /// Get the latest reconstitution trace
    public func getLatestReconstitutionTrace() : ?ReconstitutionTrace {
      if (state.reconstitutionTraces.size() > 0) {
        ?state.reconstitutionTraces[state.reconstitutionTraces.size() - 1];
      } else {
        null;
      };
    };

    /// Clear reconstitution traces (useful for cleanup)
    public func clearReconstitutionTraces() : () {
      state.reconstitutionTraces := [];
    };

    /// Validate current timer state consistency
    public func validateTimerState() : [Text] {
      let errors = Buffer.Buffer<Text>(0);

      // Check if nextActionId is consistent with the maximum action ID in the tree
      let ?maxAction = BTree.max(state.timeTree) else {
        // Empty tree is valid
        if (state.nextActionId != 0) {
          errors.add("nextActionId should be 0 for empty tree but is " # Nat.toText(state.nextActionId));
        };
        return Buffer.toArray(errors);
      };

      if (maxAction.0.id >= state.nextActionId) {
        errors.add("nextActionId (" # Nat.toText(state.nextActionId) # ") is not greater than max action ID (" # Nat.toText(maxAction.0.id) # ")");
      };

      // Check action index consistency
      let treeSize = BTree.size(state.timeTree);
      let indexSize = Map.size(state.actionIdIndex);
      if (treeSize != indexSize) {
        errors.add("timeTree size (" # Nat.toText(treeSize) # ") does not match actionIdIndex size (" # Nat.toText(indexSize) # ")");
      };

      // Verify each action in tree is in index
      let ?minAction = BTree.min(state.timeTree) else {
        return Buffer.toArray(errors);
      };

      var actionsToValidate = BTree.scanLimit<ActionId, Action>(state.timeTree, ActionIdCompare, minAction.0, maxAction.0, #fwd, 1000);

      for ((actionId, action) in actionsToValidate.results.vals()) {
        switch (Map.get(state.actionIdIndex, Map.nhash, actionId.id)) {
          case (?time) {
            if (time != actionId.time) {
              errors.add("ActionId " # Nat.toText(actionId.id) # " has mismatched time in index (" # Nat.toText(time) # ") vs tree (" # Nat.toText(actionId.time) # ")");
            };
          };
          case (null) {
            errors.add("ActionId " # Nat.toText(actionId.id) # " is in tree but not in index");
          };
        };
      };
    ...
    /// Get comprehensive diagnostics about the timer system
    public func getTimerDiagnostics() : TimerDiagnostics {
      let now = get_time();
      var overdueCount = 0;
      ...
      let ?minAction = BTree.min(state.timeTree) else {
        return {
          ...
          pendingActions = 0;
          overdueActions = 0;
          systemTimerStatus = state.nextTimer;
          lastExecutionDelta = Int.abs(Time.now()) - state.lastExecutionTime;
          nextExecutionDelta = null;
          lockStatus = state.timerLock;
        };
      };
      ...
      var actionsToCheck = BTree.scanLimit<ActionId, Action>(state.timeTree, ActionIdCompare, minAction.0, { time = now; id = 0 }, #fwd, 1000);

      for ((actionId, action) in actionsToCheck.results.vals()) {
        if (actionId.time <= now) {
          overdueCount += 1;
        };
      };

      let nextExecutionDelta = switch (state.expectedExecutionTime) {
        case (?time) ?((time : Int) - Int.abs(Time.now()));
        case (null) null;
      };

      {
        currentTime = now;
        totalActions = state.nextActionId;
        pendingActions = BTree.size(state.timeTree);
        overdueActions = overdueCount;
        systemTimerStatus = state.nextTimer;
        lastExecutionDelta = Int.abs(Time.now()) - state.lastExecutionTime;
        nextExecutionDelta = nextExecutionDelta;
        lockStatus = state.timerLock;
      };
    };

    // Utility Functions for Forceful Timer Cancellation

    /// Cancel multiple actions by filter criteria
    public func cancelActionsByFilter<system>(filter : ActionFilter) : CancellationResult {
      let cancelled = Buffer.Buffer<ActionId>(0);
      let notFound = Buffer.Buffer<Nat>(0);
      let errors = Buffer.Buffer<(Nat, Text)>(0);
      ...
      var actionsToProcess = BTree.scanLimit<ActionId, Action>(state.timeTree, ActionIdCompare, minAction.0, maxAction.0, #fwd, 10000);

      for ((actionId, action) in actionsToProcess.results.vals()) {
        let shouldCancel = switch (filter) {
          case (#ByType(actionType)) action.actionType == actionType;
          case (#ByTimeRange(start, end)) actionId.time >= start and actionId.time <= end;
          case (#ByRetryCount(retries)) action.retries >= retries;
          case (#ByActionId(id)) actionId.id == id;
          case (#All) true;
        };
      ...
      {
        cancelled = Buffer.toArray(cancelled);
        notFound = Buffer.toArray(notFound);
        errors = Buffer.toArray(errors);
      };
    };

    /// Emergency function to cancel ALL timers and clear the system
    public func emergencyClearAllTimers<system>() : Nat {
      let count = BTree.size(state.timeTree);

      // Clear all entries from the tree and index
      let ?minAction = BTree.min(state.timeTree) else {
        return 0;
      };
      let ?maxAction = BTree.max(state.timeTree) else {
      ...
      var actionsToDelete = BTree.scanLimit<ActionId, Action>(state.timeTree, ActionIdCompare, minAction.0, maxAction.0, #fwd, 10000);

      for ((actionId, action) in actionsToDelete.results.vals()) {
        ignore BTree.delete(state.timeTree, ActionIdCompare, actionId);
        ignore Map.remove(state.actionIdIndex, Map.nhash, actionId.id);
      };

      // Cancel the system timer if it exists
      switch (state.nextTimer) {
        case (?timerId) {
          Timer.cancelTimer(timerId);
          state.nextTimer := null;
    ...
    /// Force cancel system timer (useful for emergency situations)
    public func forceSystemTimerCancel() : Bool {
      switch (state.nextTimer) {
        case (?timerId) {
          Timer.cancelTimer(timerId);
          state.nextTimer := null;
    ...
    /// Force release timer lock (use with caution)
    public func forceReleaseLock() : ?Time {
      let oldLock = state.timerLock;
      state.timerLock := null;
      oldLock;
    };

    /// Cancel actions by IDs with detailed results
    public func cancelActionsByIds<system>(actionIds : [Nat]) : CancellationResult {
      let cancelled = Buffer.Buffer<ActionId>(0);
      let notFound = Buffer.Buffer<Nat>(0);
      let errors = Buffer.Buffer<(Nat, Text)>(0);

      for (id in actionIds.vals()) {
        switch (Map.get(state.actionIdIndex, Map.nhash, id)) {
          case (?time) {
            let actionId = { time; id };
            removeAction(actionId);
            cancelled.add(actionId);
          };
          case (null) {
    ...
    /// Get actions matching filter for inspection before cancellation
    public func getActionsByFilter(filter : ActionFilter) : [ActionDetail] {
      let result = Buffer.Buffer<ActionDetail>(0);
