...
shared (deployer) persistent actor class TimerTool<system>(args : TT.Args) = this {
  ...
  transient let initManager = ClassPlusLib.ClassPlusInitializationManager(deployer.caller, Principal.fromActor(this), true);

  var tt_migration_state : TT.State = TT.Migration.migration.initialState;

  D.print("about to call init");
  transient let timerTool = TT.Init<system>({
    manager = initManager;
  ...
  // Utility functions for tracing reconstitution and forceful cancellation

  public query func get_reconstitution_traces() : async [TT.ReconstitutionTrace] {
    timerTool().getReconstitutionTraces();
  };

  public query func get_latest_reconstitution_trace() : async ?TT.ReconstitutionTrace {
    timerTool().getLatestReconstitutionTrace();
  };

  public shared func validate_timer_state() : async [Text] {
    timerTool().validateTimerState();
  };

  public query func get_timer_diagnostics() : async TT.TimerDiagnostics {
    timerTool().getTimerDiagnostics();
  };

  public shared func cancel_actions_by_filter(filter : TT.ActionFilter) : async TT.CancellationResult {
    timerTool().cancelActionsByFilter<system>(filter);
  };

  public shared func cancel_actions_by_ids(actionIds : [Nat]) : async TT.CancellationResult {
    timerTool().cancelActionsByIds<system>(actionIds);
  };

  public query func get_actions_by_filter(filter : TT.ActionFilter) : async [TT.ActionDetail] {
    timerTool().getActionsByFilter(filter);
  };

  public shared func emergency_clear_all_timers() : async Nat {
    timerTool().emergencyClearAllTimers<system>();
  };

  public shared func force_system_timer_cancel() : async Bool {
    timerTool().forceSystemTimerCancel();
  };

  public shared func force_release_lock() : async ?Nat {
    timerTool().forceReleaseLock();
  };

  public shared func clear_reconstitution_traces() : async () {
    timerTool().clearReconstitutionTraces();
  };
