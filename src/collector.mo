...
shared (deployer) persistent actor class Collector<system>() = this {
  ...
  /**
    ...
    */
  public func icrc85_deposit_cycles(request : ShareArgs) : async {
    #Ok : Nat;
    #Err : ShareCycleError;
  } {
  ...
  /**
    ...
    * @returns {Nat} - The amount of cycles accepted.
    */
  public func icrc85_deposit_cycles_notify(request : ShareArgs) : () {
    D.print("recived cycles");
    let amount = Cycles.available();
    let accepted = amount;
    ignore Cycles.accept<system>(accepted);
  ...
  public shared (msg) func wallet_withdraw(amount : Nat, owner : ?Principal) : async DepositResult {
